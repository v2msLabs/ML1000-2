...
data("USArrests") # Standardize the data df <- scale( USArrests)

df = USArrests 

dfs = df %>% scale()

res.dist <- dist( dfs, method = "euclidean")
...
agnesCont = factoextra::hcut(victimStatsCont, k = 10,  hc_func ="agnes", 
                         hc_method = "ward.D2", hc_metric = "euclidean",stand = F)

agnesDataCont = cbind(victimStats,agnesCont$cluster)

agnesDataCont2 = subset(agnesDataCont,select = c(agnesCont$cluster))
