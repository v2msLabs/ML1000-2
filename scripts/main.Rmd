...
The objective of this research is to discover trends and tendancies in the gun violence developmen in the US employing unsupervised learning approaches.
...
We decided to select CLARA method because it scales well and can deal with continuous and categorical data. It is based on The **Partitioning Around Medoids** (*PAM*) algorithm, which is a popular realization of k-medoids clustering. We start with univariate data set for simplicity. Then we will take a look at silhouette plot to analyze the result. The silhouette coefficient measure how well the clusters are separated. The silhouette analysis also provides insights into the cluster density.

Let's see how *n_victim* feature could be split by *CLARA*. The method also scales the data.

```{r plot_clara1, echo = FALSE, fig.align="center", fig.cap="CLARA Approach. Univariate Feature, 4 Cluster Silhouette", fig.height = 4}
...
```{r plot_clara2, echo = FALSE, fig.align="center", fig.cap="CLARA Approach. Number of Victims/Cluster Scatter Plot", echo = FALSE}
...
```{r plot_clara3, fig.align="center", fig.cap="CLARA Approach. Number of Victims Clusters on Map", echo = FALSE}
...
```{r plot_clara4, echo = FALSE, fig.align="center", fig.cap="CLARA Approach. Two Feature, 4 Cluster Silhouette", , fig.height = 4}
...
```{r plot_clara5, fig.align="center", fig.cap="CLARA Approach. Two Continuous Feature, 10 Cluster Silhouette"}
...
```{r plot_clara6, fig.align="center", fig.cap="CLARA Approach. Two Continuous Feature, 10 Cluster Silhouette. Manhattan Metrics"}
...
```{r plot_clara7, fig.align="center", fig.cap="CLARA Approach. Two Continuous Feature, 10 Cluster Plot. Manhattan Metrics"}
...
```{r plot_clara8, fig.align="center", fig.cap="CLARA Approach. Two Continuous Feature, 10 Clusters. Manhattan Metrics. Number of Victims and Guns Clusters on Map"}
combined = cbind(victimStats, cluster=c(clara$clustering))
ggplot(victimStats, aes(x=longitude, y=latitude )) + 
...
```{r plot_clara9, fig.align="center", fig.cap="CLARA Approach. Number of Victims and Guns Cluster 3D Plot", echo = FALSE}
...
```{r plot_anges_s1, fig.align="center", fig.cap="AGNES Approach. 4 Cluster, Univariate Feature Silhouette", echo = FALSE}
victimStats = data %>% filter(n_victims >= 3 & place_type>0) %>% arrange(date)
...
fviz_silhouette(agnes, title = "") +
  theme(axis.text.x = element_blank(), axis.ticks.x = element_blank())
```
Following our routine we review the silhouette coefficients. They are less satisfactory than the ones produced by *CLARA*. Further experimentation with the number of the clusters and distance metrics did yield better result. So we are moving on to the next feature set: *n_victims* and *n_guns_involved*

```{r plot_anges_s2, fig.align="center", fig.cap="AGNES Approach. 4 Cluster, Two Feature Silhouette", echo = FALSE}
victimStats = data %>% filter(n_victims >= 3 & n_guns_involved>0) %>% arrange(date)
stats =  victimStats %>% select(n_victims, n_guns_involved)
...
```{r plot_dbscan1, fig.align="center", fig.cap="DBSCAN Approach. Finding Hyper-Parameters", echo = FALSE}
victimStats = data %>% filter(n_victims >= 3 & n_guns_involved > 0) %>% arrange(date)
stats =  victimStats %>% select(n_victims, n_guns_involved) %>% scale()
dbscan::kNNdistplot(stats, k = 5) 
```

The plot (Figure: \ref{fig:plot_dbsacan1}) has quite sharp elbow. It appears that **eps** = 1.5 would be a good starting value for 5 minimum points.  Let's run dbscan method

```{r plot_dbscan2, fig.align="center", fig.cap="DBSCAN Approach. Clusters", echo = FALSE}
...
```{r plot_dbscan3, fig.align="center", fig.cap="DBSCAN Approach. Finding Hyper-Parameters for High-dimentional Dataset", echo = FALSE}
...
```{r plot_dbscan4, fig.align="center", fig.cap="DBSCAN Approach. Clusters", echo = FALSE}
...
```{r plot_dbscan5, fig.align="center", fig.cap="DBSCAN Approach. Victims of Gun Violence on the Map", echo = FALSE}
